...
class QueueManagerSpec extends NeoBaseSpec
{
    ...
    public function it_should_have_an_empty_array_of_queues_by_default()
    {
        $this->getQueues()->shouldHaveCount(0);
    }

    public function it_should_return_bool_if_queue_exist_for_connection()
    {
        $this->shouldNotHaveQueue($this->getConnection());
    }

    public function it_should_create_queue_for_connection_if_no_queue_exist()
    {
    ...
    public function it_should_return_a_queue_for_a_connection()
    {
        $this->createQueue($this->getConnection());
        $this->getQueue($this->getConnection())->shouldHaveType('NeoConnect\Queue\Queue');
    }

    public function it_should_throw_error_if_queue_for_connection_not_exist_when_demanding_queue()
    {
        $this->shouldThrow('\InvalidArgumentException')->duringGetQueue($this->getConnection());
    }

    public function it_should_return_the_queue_if_alias_exist_or_not()
    {
        $this->createQueue($this->getConnection());
        $this->getQueues()->shouldHaveCount(1);
        $this->createQueue($this->getConnection());
    ...
    public function it_should_return_queue_state_code_for_connection()
    {
    ...
    public function it_should_return_queue_for_string_connection_alias()
    {
    ...
    public function it_should_handle_a_get_queue_for_statement_event()
    {
        $ev = $this->getEvent();
    ...
    public function it_should_subscribe_to_the_queue_kernel_event()
    {
